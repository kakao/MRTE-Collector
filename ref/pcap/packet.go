...
func (p *Packet) GetSourceDestination() []string {
	// If there's just one header, return that.
	if len(p.Headers) == 1 {
		if hdr, ok := p.Headers[0].(addrHdr); ok {
			return []string{string(p.Time.Unix()), hdr.SrcAddr(), hdr.DestAddr()}
		}
	}
	...
	if len(p.Headers) >= 2 {
		// Commonly the first header is an address.
		if addr, ok := p.Headers[0].(Iphdr); ok {
			if hdr, ok := p.Headers[1].(Tcphdr); ok {
				return []string{string(p.Time.Unix()), addr.SrcAddr() + ":" + hdr.SrcAddr(), addr.DestAddr() + ":" + hdr.DestAddr()}
			}
		}
	}
...
func (p *Packet) IsUserDataPacket() bool {
	for i := 0; i < len(p.Headers); i++ {
		if p.Headers[i].(addrHdr).IsUserData() {
			return true
		}
	}
	return false
